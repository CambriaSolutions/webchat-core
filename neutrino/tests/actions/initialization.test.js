...
afterEach(() => {
  ...
  it('should set up dialog flow with correct options', () => {
    ...
    expect(storedActions[4].type).toEqual('UPDATE_CURRENT_TIME' );
    expect(storedActions[5]).toEqual({ type: 'INITIATE_LOADING' });
   // expect(storedActions[6].type).toEqual('CREATE_THEME');
    expect(storedActions[6]).toEqual({ type: 'SHOW_WINDOW' });
    expect(storedActions[7]).toEqual({ type: 'FULLSCREEN' });
  });
  ...
  it('should set up dialog flow with correct options', () => {
    ...
    expect(storedActions[5]).toEqual({ type: 'INITIATE_LOADING' });
   // expect(storedActions[6].type).toEqual('CREATE_THEME');
    expect(storedActions[6]).toEqual({ type: 'HIDE_WINDOW' });
